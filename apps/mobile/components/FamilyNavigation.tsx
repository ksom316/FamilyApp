...
export function MobileFamilyNavigation() {
  ...
  return (
    <View style={[styles.bottomNav, { backgroundColor: theme.surface, borderColor: theme.border, paddingBottom: Math.max(insets.bottom, spacing.sm) }]}>
      {items.map((item) => {
        const active = pathname.endsWith(`/${item.name}`) || (item.name === 'more' && ['chat', 'private-chat', 'memories', 'capsules', 'location', 'brain', 'invite'].some((route) => pathname.split('/').includes(route)));
        return (
