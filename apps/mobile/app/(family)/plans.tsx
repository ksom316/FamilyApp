...
function dateTimeValue(date = new Date(Date.now() + 60 * 60 * 1000)) {
        ...
        <View style={[styles.planColumns, width >= 1120 && styles.planColumnsWide]}>
          <View style={styles.planColumn}>
            ...
            {plans.events.length ? <View style={styles.list}>{plans.events.map((event) => {
              ...
              return <EventCard key={event.id} event={event} canManage={canManage} busy={busyId === event.id} onEdit={() => setEditor({ kind: 'event', item: event })} onDelete={() => confirmDelete('event', () => runAction(event.id, () => deleteFamilyEvent(family.familyId, event.id)))} />;
          ...
          <View style={styles.planColumn}>
            ...
            {plans.tasks.length ? <View style={styles.list}>{plans.tasks.map((task) => {
              ...
              return <TaskCard key={task.id} task={task} canManage={canManage} canComplete={canComplete} busy={busyId === task.id} onToggle={() => void runAction(task.id, () => setFamilyTaskCompletion(family.familyId, task.id, !task.completedAt))} onEdit={() => setEditor({ kind: 'task', item: task })} onDelete={() => confirmDelete('task', () => runAction(task.id, () => deleteFamilyTask(family.familyId, task.id)))} />;
...
function EventEditor({ familyId, event, onCancel, onSaved }: { familyId: string; event?: FamilyEvent; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  const [startsAt, setStartsAt] = useState(event ? dateTimeValue(new Date(event.startsAt)) : dateTimeValue());
  const [endsAt, setEndsAt] = useState(event?.endsAt ? dateTimeValue(new Date(event.endsAt)) : '');
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  async function save() {
    const start = toIso(startsAt); const end = endsAt ? toIso(endsAt) : undefined;
    if (!start || (endsAt && !end)) return setError('Use a valid date and time, such as 2026-10-03T18:30.');
    setSaving(true); setError(null);
    try { const input = { title, description, startsAt: start, endsAt: end ?? undefined }; if (event) await updateFamilyEvent(familyId, event.id, input); else await createFamilyEvent(familyId, input); await onSaved(); }
    catch (error) { setError(error instanceof PlansApiError ? error.message : 'The event could not be saved.'); }
    finally { setSaving(false); }
  }
  return <Card elevated style={styles.editor}><AppText variant="heading">{event ? 'Edit event' : 'Add an event'}</AppText><TextField label="Title" value={title} onChangeText={setTitle} maxLength={140} autoFocus /><TextField label="Description (optional)" value={description} onChangeText={setDescription} multiline /><View style={styles.formRow}><View style={styles.formField}><TextField label="Starts" value={startsAt} onChangeText={setStartsAt} hint="YYYY-MM-DDTHH:mm" autoCapitalize="none" /></View><View style={styles.formField}><TextField label="Ends (optional)" value={endsAt} onChangeText={setEndsAt} hint="YYYY-MM-DDTHH:mm" autoCapitalize="none" /></View></View>{error ? <AppText variant="caption" tone="danger" style={styles.formError}>{error}</AppText> : null}<View style={styles.formActions}><Button label="Cancel" variant="quiet" onPress={onCancel} /><Button label={event ? 'Save event' : 'Create event'} loading={saving} onPress={() => void save()} /></View></Card>;
}
...
function TaskEditor({ familyId, task, members, onCancel, onSaved }: { familyId: string; task?: FamilyTask; members: FamilyMember[]; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  const [dueAt, setDueAt] = useState(task ? dateTimeValue(new Date(task.dueAt)) : dateTimeValue());
